if (document.getElementById("uploader_form")) {
  /* Uploader */
  ...
  function uploadMessage(response, text) {
    ...
    if (["image/jpeg", "image/png", "image/gif"].indexOf(response.mime) == -1) {
      icon = response.mime.replace("/", "-") + ".svg";
      if (delfticons.indexOf(icon) < 0) {
        icon = response.mime.startsWith("video/")
          ? "video-x-generic.svg"
          : "none.svg";
      }
    ...
    data.append(
      "headers",
      new Blob([JSON.stringify(headers)], { type: "application/json" }),
    );
  ...
  function upload_files(index) {
    ...
    xhr.upload.addEventListener("loadstart", function (event) {
      ...
      progress_filename.innerHTML =
        (items > 1 ? "(" + (index + 1) + "/" + items + ") " : "") + file.name;
    });
  ...
  paste_form.addEventListener("submit", function (event) {
    ...
    data.append("data", new Blob([paste_text.value], { type: "text/plain" }));
  ...
  paste_text.addEventListener("keydown", function (event) {
    ...
    if (key == 9 && !event.shiftKey && !event.ctrlKey && !event.altKey) {
      ...
      if (this.setSelectionRange) {
        ...
        this.value =
          this.value.substring(0, selection_start) +
          "\t" +
    ...
    xhr.onload = function () {
      switch (this.status) {
        case 200:
          setMessage(
            "notice",
            "Settings successfully updated. " + "You should refresh the page.",
            true,
          );
      ...
      switch (this.status) {
        case 200:
          ...
          [].forEach.call(response.links, function (item) {
            link_history.insertAdjacentHTML(
              "beforeend",
        ...
        xhr.onload = function () {
          switch (this.status) {
            case 200:
              ...
              break;
            default:
              setMessage("error fatal", "Could not delete item.");
              break;
          }
        };
        xhr.send(data);
  ...
  function prune_unused() {
    var confirmed = window.confirm(
      "Are you sure you want to prune all " + "unused links?",
    );
    if (!confirmed) {
      return;
    }
